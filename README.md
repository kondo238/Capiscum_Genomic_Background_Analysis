...
(A) Preparation of catalog dataset using stacks pipeline. Stacks pipeline firstly generates stacks (genomic fragments) by de novo assembly of RAD-seq reads of each sample. Then, the catalogs (loci) are defined based on sequence similarity among all stacks, and SNPs in stacks among all samples are detected for each catalog. Finally, this pipeline output catalog dataset, a matrix containing SNP genotypes including missing for all catalogs and samples. (B) characterization of stacks derivation analysis with ‘catalog dataset’. In this analysis, two arrays were prepared. The one was the ‘reference array’ showing presence-absence information of the stacks at all catalogs in five Capsicum species (C. annuum (C.a), C. chinense (C.c), C. frutescens (C.f), C. baccatum (C.b), and C. pubescens (C.p)), based on the subset of the catalog dataset (subset A). The other was the ‘test array’ including ‘presence’ information for the catalogs existed in six lines of Dalle Khursani (Dalle Khursani-possessing catalogs), based on other subset of the catalog dataset (subset B). With the ‘merged array’ generated by combining both arrays, derivation of the stacks at Dalle Khursani-possessing catalogs was characterized based on the commonality of the catalogs with five species and three species of C. annuum-complex (C.a, C.c, and C.f) (Analysis 1). Regarding the catalog groups shared with multiple species of C. annuum complex (1. C.a & C.c-common, 2. C.a & C.f-common, 3. C.c & C.f-common, 4. three species-common), their stacks derivations were finally determined by admixture analysis based on the SNPs in stacks sequences in each catalog group (Analysis 2). 
